...
knee_sleep_trim=0.01
foot_sleep_trim=0.01
hip_sleep_trim=0.4
...
def commandRotateA():
    knee_fl(knee_up)
    hip_fl(rotate_angle_f)
    knee_fl(knee_n)
    knee_fr(knee_up)
    hip_fr(rotate_angle_b)
    knee_fr(knee_n)
    knee_br(knee_up)
    hip_br(rotate_angle_b)
    knee_br(knee_n)
    knee_bl(knee_up)
    hip_bl(rotate_angle_f)
    knee_bl(knee_n)
    hip_flfrblbr(rotate_angle_b,rotate_angle_f,rotate_angle_b,rotate_angle_f)

def commandRotateB():
    knee_fl(knee_up)
    hip_fl(rotate_angle_b)
    knee_fl(knee_n)
    knee_fr(knee_up)
    hip_fr(rotate_angle_f)
    knee_fr(knee_n)
    knee_br(knee_up)
    hip_br(rotate_angle_f)
    knee_br(knee_n)
    knee_bl(knee_up)
    hip_bl(rotate_angle_b)
    knee_bl(knee_n)
    hip_flfrblbr(rotate_angle_f,rotate_angle_b,rotate_angle_f,rotate_angle_b)
...
def keyboardInputMultiplex(c):
    ...
    if pendingCalls > 0:
        print("Command in progress, ignoring", c)
        return "busy"
    ...
    if c == "m":
        ...
        return "quit"
    if c == "z":
        threading.Thread(target=lambda: (commandAllZero(), globals().update({'pendingCalls': 0})), daemon=True).start()
    elif c == "f":
        threading.Thread(target=lambda: (commandMaxFeet(), globals().update({'pendingCalls': 0})), daemon=True).start()
    elif c == "g":
        threading.Thread(target=lambda: (commandMinFeet(), globals().update({'pendingCalls': 0})), daemon=True).start()
    elif c == "o":
        threading.Thread(target=lambda: (commandMaxKnee(), globals().update({'pendingCalls': 0})), daemon=True).start()
    elif c == "p":
        threading.Thread(target=lambda: (commandMinKnee(), globals().update({'pendingCalls': 0})), daemon=True).start()
    elif c == "d":
        threading.Thread(target=lambda: (dwalk(2), globals().update({'pendingCalls': 0})), daemon=True).start()
    elif c == "w":
        threading.Thread(target=lambda: (walk(), globals().update({'pendingCalls': 0})), daemon=True).start()
    elif c == "e":
        threading.Thread(target=lambda: (rotate("Right"), globals().update({'pendingCalls': 0})), daemon=True).start()
    elif c == "q":
        threading.Thread(target=lambda: (rotate("Left"), globals().update({'pendingCalls': 0})), daemon=True).start()
    elif c == "l":
        threading.Thread(target=lambda: (stand("up"), globals().update({'pendingCalls': 0})), daemon=True).start()
    else:
        pendingCalls = 0
